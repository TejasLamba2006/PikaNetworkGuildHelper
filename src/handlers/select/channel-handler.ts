...
export default async function channelSelectHandler(client: Client) {
  const directories = await fs.readdir("./src/interactions/select-menus/channel");
  for (const directory of directories) {
    const files = (
      await fs.readdir(`./src/interactions/select-menus/channel/${directory}/`)
    ...
    for (const file of files) {
      ...
      client.logger.info(`[SELECTS] Loaded channel select menu ${imported.customId}`);
    }
  }
}
