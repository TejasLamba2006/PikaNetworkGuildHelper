...
export default async function roleSelectHandler(client: Client) {
  const directories = await fs.readdir("./src/interactions/select-menus/role");
  for (const directory of directories) {
    const files = (await fs.readdir(`./src/interactions/select-menus/role/${directory}/`)).filter(
    ...
    for (const file of files) {
      ...
      client.logger.info(`[SELECTS] Loaded role select menu ${imported.customId}`);
    }
  }
}
