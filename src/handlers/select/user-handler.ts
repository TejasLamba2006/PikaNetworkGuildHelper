...
export default async function userSelectHandler(client: Client) {
  const directories = await fs.readdir("./src/interactions/select-menus/user");
  for (const directory of directories) {
    const files = (await fs.readdir(`./src/interactions/select-menus/user/${directory}/`)).filter(
    ...
    for (const file of files) {
      ...
      client.logger.info(`[SELECTS] Loaded user select menu ${imported.customId}`);
    }
  }
}
