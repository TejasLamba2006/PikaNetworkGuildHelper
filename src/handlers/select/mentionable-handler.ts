...
export default async function mentionableSelectHandler(client: Client) {
  const directories = await fs.readdir("./src/interactions/select-menus/mentionable");
  for (const directory of directories) {
    const files = (
      await fs.readdir(`./src/interactions/select-menus/mentionable/${directory}/`)
    ...
    for (const file of files) {
      ...
      client.logger.info(`[SELECTS] Loaded mentionable select menu ${imported.customId}`);
    }
  }
}
