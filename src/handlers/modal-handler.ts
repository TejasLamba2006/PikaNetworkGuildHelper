...
export default async function modalHandler(client: Client) {
  const directories = await fs.readdir("./src/interactions/Modals");
  for (const directory of directories) {
    const files = (await fs.readdir(`./src/interactions/Modals/${directory}/`)).filter(
      v => !v.endsWith(".disabled.ts")
    );
    for (const file of files) {
      const imported = (await import(`../interactions/Modals/${directory}/${file}`).then(
